...
def unzip_and_gather_files():
    ...
    zip_path = input("Please provide the path to the zipped file: ")
...
class Accelerator:
    ...
    def JLABaccelerator(self):
        while True:
            user_task = input("[Run sequence files from START to END = 1], [Twiss = 2], [Error Analysis = 3], [Exit = 4]:  ").strip()
            if user_task == '1':
                ...
                inputSTART = input("Name the file you would like to run Error Analysis on: ")
...
if __name__ == "__main__":
    ...
    # Ask the user if they want to unzip a file and modify paths
    unzip_choice = input("Would you like to unzip a file? (y/n): ").strip().lower()
    if unzip_choice == 'y':
        unzip_and_gather_files()
        modify_lattice_paths()
    elif unzip_choice == 'n':
        print("Skipping file unzip and lattice modification...")

    # Prompt for the accelerator type after handling unzipping and path modification
    print('Name of Accelerator: SLAC = 1 , JLAB = 2, TRIUMF = 3')
    inputNAME = int(input())
